...
def load_assets():
    ...
    def load(f, mode="rb"):
        return (pickle.load(open(os.path.join(base,f),mode))
                if mode=="rb"
                else json.load(open(os.path.join(base,f))))
    return (load("final_model.pkl"), load("scaler_final.pkl"),
            load("bootstrap_ensemble.pkl"),
            load("shap_weights.json","r"),
            load("feature_stats.json","r"),
            load("compliance_info.json","r"))
...
LANG = {
  "en":{
    ...
    "unknown":"Unknown","completeness":"Data completeness",
    "feature_tab":"Feature Contribution","percentile_tab":"Percentile Context",
  ...
  "ur":{
    ...
    "unknown":"معلوم نہیں","completeness":"ڈیٹا مکمل ہونا",
    "feature_tab":"خصوصیت","percentile_tab":"پرسینٹائل",
...
def check_ood(feat, val):
    ...
    return bool(s and (val<s["p5"] or val>s["p95"]))
...
def draw_gauge(value, limit, label):
    ...
    ax.plot(0.5,0,"o",color="white",markersize=6,zorder=5)
    col = ("#10b981" if value<=limit*0.75 else
           "#f59e0b" if value<=limit else "#ef4444")
...
with st.sidebar:
    ...
    confidence   = st.slider("Prediction interval confidence",0.80,0.99,0.95,0.01)
...
with st.expander(f"  {L['how_to']}", expanded=False):
    st.markdown("""
- **Enter your field readings** below. Tick **Unknown** for any unavailable value -- the model still predicts using remaining inputs.
- **TSS is the most important parameter** (55% predictive weight). If only one measurement is possible, prioritise TSS.
...
if predict_btn:
    ...
    raw = {"pH":ph_val,"Temperature (°C)":temp_val,"DO (mg/L)":do_val,
           ...
           "Monsoon_Flag":1 if is_monsoon else 0,"Rain_Event_Flag":rain_flag}
    ...
    ood = [c for c in FEATURE_COLS
           if not unk_map.get(c,False) and raw.get(c) is not None
           and check_ood(c, raw[c])]
    ...
    with r1:
        ...
        st.caption(f"{int(confidence*100)}% Interval: {bl:.1f} -- {bh:.1f} mg/L")
    ...
    with r2:
        ...
        st.caption(f"{int(confidence*100)}% Interval: {cl:.1f} -- {ch:.1f} mg/L")
        st.progress(min(cc,1.0),text=f"{L['completeness']}: {cc*100:.0f}%")

    t1,t2,t3 = st.tabs([L["feature_tab"],L["percentile_tab"],L["cost_tab"]])
    with t1:
        if show_feature:
            sw   = shap_weights["bod_weights"]
            sp   = sorted(zip(sw.keys(),sw.values()),key=lambda x:x[1])
            cols = ["#ef4444" if f in missing else "#3b82f6" for f,_ in sp]
            fig,ax = plt.subplots(figsize=(8,4))
            ax.barh([f for f,_ in sp],[v for _,v in sp],color=cols,alpha=0.85)
    ...
    with t2:
        pm = {"pH":ph_val,"Temperature (°C)":temp_val,"DO (mg/L)":do_val,
              "Turbidity (NTU)":turb_val,"TDS (mg/L)":tds_val,
              "EC (µS/cm)":ec_val,"TSS (mg/L)":tss_val}
        sk = {"pH":"pH","Temperature (°C)":"Temperature","DO (mg/L)":"DO",
              "Turbidity (NTU)":"Turbidity","TDS (mg/L)":"TDS",
              "EC (µS/cm)":"EC","TSS (mg/L)":"TSS"}
        st.caption("Shows where each entered value sits within the training data distribution.")
        for feat,val in pm.items():
            s = feat_stats.get(sk[feat],{})
            if val is None or not s:
                st.caption(f"{feat}: not provided"); continue
            pct = min(100,max(0,(val-s["min"])/max(s["max"]-s["min"],1)*100))
            st.caption(f"{feat}: **{val:.1f}** ({pct:.0f}th percentile)")
            st.progress(pct/100)
